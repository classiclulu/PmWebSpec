...
//          error_log('Cannot execute '.$this->db->last_query(), 3, base_url().'logs/sql_errors.log');
    ...
    public function getTypeLabel($type) {
        ...
        if ($type == 'PPK-standard') {
            ...
            $label = 'Data for Population PK Analysis';
            $dataset_sorted = 'STUDYID, USUBJID, AFRELTM, EVID';
        }else if ($type == 'ER-safety') {
            $label = 'Exposure-response safety';
            $dataset_sorted = 'STUDYID, USUBJID';
        } else if ($type == 'ER-efficacy') {
            $label = 'Exposure-response efficacy';
            $dataset_sorted = 'STUDYID, USUBJID';
        } else if ($type == 'TGD') {
            $label = 'Tumor growth dynamics';
            $dataset_sorted = 'STUDYID, USUBJID';
        } else if ($type == 'PKPD') {
            $label = 'Pharmacokinetics and pharmacodynamics';
            $dataset_sorted = 'STUDYID, USUBJID';
        } else if ($type == 'ER-safety-efficacy') {
            $label = 'Exposure-response safety and efficacy'; 
            $dataset_sorted = 'STUDYID, USUBJID';       
        }

        //////////////
        //PPK i think coverd in above itslef

        //PKPD
        else if ($type == 'PKPD-ISOP') {
            $label = 'Pharmacokinetics and pharmacodynamics';
            $dataset_sorted = 'STUDYID, USUBJID';
        }
        ...
        //ER
            ...
            $label = 'Exposure-response safety and efficacy'; 
            $dataset_sorted = 'STUDYID, USUBJID';      
        }else if ($type == 'ER-ISOP-safety') {
            $label = 'Exposure-response safety';
            $dataset_sorted = 'STUDYID, USUBJID';
        } else if ($type == 'ER-ISOP-efficacy') {
            $label = 'Exposure-response efficacy';
            $dataset_sorted = 'STUDYID, USUBJID';
        }
    ...
    public function tempVarsQuery($type, $ppk1, $er1, $other1) {
        ...
        //print_r($type);exit;
        $isop=["PPK-ISOP-RELA", "PPK-ISOP-IPI", "PPK-ISOP-NIVO", "ER-ISOP-safety", "ER-ISOP-efficacy", "ER-ISOP-safety-efficacy","PKPD-ISOP"];

         $ERisop=["ER-ISOP-safety", "ER-ISOP-efficacy", "ER-ISOP-safety-efficacy"];
