...
<?php
    ...
    if(!isset($param) && $param == NULL && strtolower($param) != 'version') {
		...
		<fieldset>
                ...
                <div class="row">
                  <div class="col col-md-2">
				   ...
				   <div class="col col-md-2">
					...
					<input style ="width:100%" type="text" name="cname"  id="cname" onkeyup="myFunction()" placeholder="Enter Compund Ex: BMS-">
					</div>
        	...
        	<tbody>			  
        		<tr>
        			<th  onclick="sortTable(0)"> Specification ID </th>
        			<th  onclick="sortTable(1)"> Created By </th>
        			<th style="width: 400px;" onclick="sortTable(2)"> Dataset Path </th>
        			<th style="width: 150px;" onclick="sortTable(3)"> Indication </th>
        			<th style="width: 150px;" onclick="sortTable(4)"> Dataset Type </th>	
        			<th style="width: 150px;" onclick="sortTable(5)"> Creation Date </th>				
		...
		//echo $this->session->userdata('selection');exit;$spec['approved']
           // echo $this->session->userdata('selection');exit;
        if(!empty($all_specs)) {
            foreach($all_specs as $indx => $spec) {
				if(($this->session->userdata('selection') == 'ImportExisting') || ($this->session->userdata('selection')=='ReviewApprove') || ($this->session->userdata('selection') == 'ExportEsub') || ($this->session->userdata('selection')=='Download') ) {
					echo '<tr>';
    ...
    if((isset($param) && true == in_array($param, array('version', NULL))) && !empty($spec_info['spec_gnrl'])) {
        ...
        echo '<tbody>
                <tr>
                    ...
                    <th style="width: 30%;" onclick="sortTable(5)">Changes made</th>
                </tr>
        	  </tbody>';
    ...
    if(isset($param) && $param=="version") {
        ...
        } elseif (strtolower($selected)=="download") {
	...
	//print_r($all_specs);
    if (isset($param) && $param=="version" && strtolower($selected)=="download") {
    	echo '<div class="row">';
...
<script type="text/javascript">
    ...
    function myFunction() {
    	...
    	for (i = 1; i < table.rows.length; i++) {
    		...
    		var td2 = table.rows[i].cells[8].innerHTML;
    		var td3 = table.rows[i].cells[4].innerHTML;
    		var td4 = table.rows[i].cells[1].innerHTML;
    		var td5 = table.rows[i].cells[3].innerHTML;

    		if (td1.toUpperCase().indexOf(filter1) > -1 && td2.toUpperCase().indexOf(filter2) > -1 && td3.toUpperCase().indexOf(filter3) > -1 && td4.toUpperCase().indexOf(filter4) > -1 && td5.toUpperCase().indexOf(filter5) > -1 ) {
