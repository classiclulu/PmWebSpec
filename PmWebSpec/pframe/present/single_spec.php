			...
			<div class="container-fuild">
				<div class="row">
					<div class="col col-md-6 pull-left">
						<label><b>Your Full Name: (First Last)</b></label><input id="username" name="username" type="text" size="20" value="<?php echo $full_name; ?>" required="required" />
					</div>

					<div class="col col-md-6 pull-right">
						<label><b>Compound Name: (e.g. BMS-936558, cc-12345)</b></label><input id="cname" name="cname" type="text" size="20" value="<?php echo $cname; ?>" title="Compound name must start with letter ex: BMS- and followed by the compound number" oninput="cnameCheck();" required="required" />
					</div>
	...
	<div class="title-info">
		<fieldset>
			...
			<div id="General Information" class="tabcontent" name="General Information">
				...
				<div class="panel  required">
					...
					<div class="row required">
						<div class="col-25"><label>Analysis title (eg. Population Pharmacokinetic Analysis of BMS-123456) : </label></div>
						<div class="col-75"><input id="title" name="title" type="text" size="80" /></div>
					</div>
					<div class="row required">
						<div class="col-25"><label>Project (BMS-123456, TA001001) : </label></div>
				...
				<div class="panel ">
					...
					<?php 
						if($spec_type == 'PPK-standard' || $spec_type == 'PPK-ISOP-RELA' || $spec_type == 'PPK-ISOP-NIVO' || $spec_type == 'PPK-ISOP-IPI') {
					...
					<div class="row">
						<div class="col-25"><label>Dataset Location  (final/derived under prd):</label></div>
					...
					<font size="2" >
						<table id="pathTable" border="5" cellpadding="5" cellspacing="5" style="background-color:#ffffff; border-collapse:collapse; border:2px solid #808B96;">
							...
							<tr>
								...
								<th style="width:280px;">level 0</th>
								<th style="width:280px;">level 1/SDTM</th>
								<th style="width:280px;">level 2/ADaM</th>
				...
				<!-- Source Data Path on PKMS -->
				<button type="button" class="accordion" onclick="openpanel();"><label>Source data path on PKMS</label></button>
				<div class="panel ">
					<br/>
					<p>Location(path) of each study:</p>
			...
			<div id="Dataset Structure" class="tabcontent" name="Dataset Structure">
				...
				} elseif($spec_type=='ER-safety' OR $spec_type=='ER-ISOP-safety') {
					echo '<b>Please select endpoints and optional variables to add:</b>';
					echo '<div class="chbox">';
					echo '<label><input id="gr3" type="checkbox" name="endpoints[]" value="gr3" class="outcheck" />
        					  Grade 3+ AE</label>
        
        				      <label><input id="aedcd" type="checkbox" name="endpoints[]" value="aedcd" class="outcheck" />
        				      AE DC/D</label>
        
        					  <label><input id="imae" type="checkbox" name="endpoints[]" value="imae" class="outcheck" />
        				      IMAE</label>
        
        				      <label><input id="gr2imae" type="checkbox" name="endpoints[]" value="gr2imae" class="outcheck" />
        				      Grade 2+ IMAE</label>
        
        				      <label><input id="drae" type="checkbox" name="endpoints[]" value="drae" class="outcheck" />
        				      Drug-related AE</label>
        
        				      <label><input id="draedcd" type="checkbox" name="endpoints[]" value="draedcd" class="outcheck" />
        				      Drug-related AE DC/D</label>
        
        				      <label><input id="drgr2ae" type="checkbox" name="endpoints[]" value="drgr2ae" class="outcheck" />
        				      Drug-related grade 2+ AE</label>
        
        				      <label><input id="drgr3ae" type="checkbox" name="endpoints[]" value="drgr3ae" class="outcheck" />
        				      Drug-related grade 3+ AE</label>';
				...
				} elseif($spec_type == 'ER-efficacy' OR $spec_type=='ER-ISOP-efficacy') {
					echo '<b>Please select endpoints and optional variables to add:</b>';
					echo '<div class="chbox">';
					echo '<label><input id="bor" type="checkbox" name="endpoints[]" value="bor" class="outcheck" />
        					  Best overall response</label>
        
        				      <label><input id="pfs" type="checkbox" name="endpoints[]" value="pfs" class="outcheck" />
        				      Progression-free survival</label>
        
        					  <label><input id="os" type="checkbox" name="endpoints[]" value="os" class="outcheck" />
        				      Overall survival</label>';
					echo '</div>';
					echo '<br/><br/>';
					echo '<div id="allspecs" class="">
        					 <table id="allspectable" name="allspectable" border="1px solid #ddd;" cellpadding="5" cellspacing="5" style="width:85%;background-color:#ffffff;border-collapse:collapse;border:2px solid #808B96">
			...
			<div id="Derivations" class="tabcontent" name="Derivations">
				...
				<div class="panel container-fluid">
					...
					<table id="deriveTable" border="5" cellpadding="5" cellspacing="5" style="width: 50%; background-color: #ffffff; border-collapse: collapse; border: 2px solid #808B96;">
						...
						?>
						<!-- <tr>
							<td><input type="checkbox" class="checkboxc"/>&nbsp;</td>
							<td><input type="text" class="struct" value="BMI"></td>
							<td style="display:none;"><input type="text" class="struct" value="weight(kg)/height(m)^2" ></td>
							<td>$$weight(kg)/height(m)^2$$</td>
						</tr>
						<tr>
							<td><input type="checkbox" class="checkboxc"/>&nbsp;</td>
							<td><input type="text" class="struct" value="BSA"></td>
							<td  style="display:none;"><input type="text" class="struct" value="0.007184*Weight(kg)^(0.425)*Height(cm)^(0.725)" ></td>
							<td>$$0.007184*Weight(kg)^{0.425}*Height(cm)^{0.725}$$</td>
						</tr>
						<tr>
							<td><input type="checkbox" class="checkboxc"/>&nbsp;</td>
							<td><input type="text" class="struct" value="CRCL"></td>
							<td  style="display:none;"><input type="text" class="struct" value="Male:(140-age(years))*Weight(kg)/(72*creat); Female: above value*0.85" ></td>
							<td>Male:$$(140-age(years))*Weight(kg) \over 72*creat$$ <br /> Female: \(above value*0.85\)</td>
						</tr>
						<tr>
							<td><input type="checkbox" class="checkboxc"/>&nbsp;</td>
							<td><input type="text" class="struct" value="GFR"></td>
							<td  style="display:none;"><input type="text" class="struct" value="GFR[ml/min/1.73m^2]=141 * min(SCr/k, 1)^a * max(SCr/k, 1)^(-1.209) * 0.993^(Age) * F1 * F2 ; k: 0.7 for females and 0.9 for males; a: -0.329 for females and -0.411 for males; F1=1.0 for males and 1.018 for females; F2=1.0 for non-blacks and 1.159 for blacks; SCr: serum creatinine in mg/dL; min: indicates the minimum of SCr/k or 1, and max indicates the maximum of SCr/k or 1" ></td>
							<td>$$GFR[ml/min/1.73m^2]=141 * min({SCr \over k}, 1)^a * max({SCr \over k}, 1)^{-1.209} * 0.993^{Age} * F1 * F2 $$ <br />k: 0.7 for females and 0.9 for males  <br />a: -0.329 for females and -0.411 for males  <br />F1=1.0 for males and 1.018 for females  <br />F2=1.0 for non-blacks and 1.159 for blacks  <br />SCr: serum creatinine in mg/dL.  <br />min: indicates the minimum of SCr/k or 1, and max indicates the maximum of SCr/k or 1
							</td>
						</tr>

						<tr>
							<td><input type="checkbox" class="checkboxc"/>&nbsp;</td>
							<td><input type="text" class="struct" value="GFR.peds"></td>
							<td  style="display:none;"><input type="text" class="struct" value="For age 1-18 year use SCHWARTZ equation (eGFR = 0.413x (height/SCr) if height is expressed in centimeters); SCr: serum creatinine in mg/dL" ></td>
							<td>For age 1-18 year use SCHWARTZ equation (eGFR = 0.413x (height/SCr) if height is expressed in centimeters). <br />SCr: serum creatinine in mg/dL.
							</td>
						</tr> -->
						</tbody>
					</table>
