...
<div class="container" style="text-align: end;margin-top: 2%;">
	...
	<form method="post"  action=<?php echo base_url('home/update/save'); ?> enctype="multipart/form-data" onsubmit="return validateMyForm();">
    	...
    	<p><font size="4" style="margin-left: 10px">Please fill out all the required field and variable name and label in the data structure table. Otherwise, you will not be able to submit your form.</font></p>
    		<fieldset>
                <?php //echo $user_spec['compound']; exit; ?>
                
    			<legend>Specification information</legend>
        		<label><b>Specification ID: </b></label><input name="spec_id" type="text" size="80" value="<?php  echo htmlspecialchars($spec_id); ?>" readonly/>
        		<label><b>Changes made: (Please track your changes to the specification)</b></label><input name="changes_made" type="text" size="80" required />
        		<label><b>Revised by: </b></label><input name="revised_by" type="text" size="80" value="<?php  echo $fullname; ?>" required  />
                 <input id="cname" name="cname" type="hidden" size="20"  value="<?php echo $user_spec['compound']; ?>" title="Compound name must start with letter Ex:BMS- and followed by the compound number" oninput="cnameCheck();" required />
    		</fieldset>
    	</div>
        ...
        <div class="title-info">	
    		...
    		<div id="General Information" class="tabcontent">
                ...
                <div class="panel container-fuild">
    				...
    				<div class="row">
    	                <div class="col-25">Modify above ,(comma) separated Data Science Programmer (CP&amp;P) :</div>
    	                <div class="col-75">
    		    ...
    		    <div class="panel container-fuild">
					...
					<?php
 						foreach($pk_data as $arr) {
 							...
 							echo '<td><select class="struct">';
 								$row = array("Wave 2","Gap","PK optimization","Legacy","Direct Data Capture","eDM","RAVE", "Other");
 								$selected = $arr['study_type'] ;
 								$i = 0;
 								while ($i < count($row)){
 							...
 							echo '</select></td>';
 							echo '<td><select class="struct">';
 								$row = array("Concentration Lock","Analysis Lock", "Other");
 								$selected = $arr['lock_type'] ;
 								$i = 0;
 								while ($i < count($row)){
 									echo "<option value='". $row[$i] ."'".($row[$i]==$selected?' selected="selected"':"").">". $row[$i]."</option>";
 										$i++;
		    	...
		    	<button type="button" class="accordion" onclick="openpanel();">Source data path on PKMS</button>
