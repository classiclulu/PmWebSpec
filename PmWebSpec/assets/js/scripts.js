		...
		function validateMyForm() {
			...
			if(cname.includes("cc") && (cname.length != 8)) {
				alert("Invalid compound name Ex. cc-12345");
				valid=false;
			}

			if(cname.includes("BMS") && (cname.length != 10)) {
				alert("Invalid compound name Ex. BMS-123456");
				valid=false;
			}

			if(cname.includes("cc") || cname.includes("BMS")) {
				
			} else {
				alert("Invalid compound name Ex. cc-12345|Ex. BMS-123456");
				valid=false;
			}
		...
		function addStudypk(){
			...
			for (var i = 0; i < selectlist.length; i++) {
					...
					option.value = selectlist[i];
					option.text = selectlist[i];
					selectbutton1.appendChild(option);
				}
		...
		function cnameCheck() {
			...
			// if (cname=="BMS-936558" && temp.includes("PPK") && temp!="PPK-NIVO") {			
			// 	alert("Please use PPK-NIVO template for Nivolumab PPK specification!");
			// 	document.getElementById("cname").value="BMS-";
			// }
		}
